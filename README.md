...
Many file formats and protocols — ACH (Automated Clearing House), fixed-width data feeds,
legacy mainframe interfaces — are restricted to printable US-ASCII characters (`0x20`–`0x7E`).
...
`CharacterCodingException` or silently substitute `?`, both of which can corrupt fixed-width
record formats that downstream processing depends on.

This library provides charsets that transliterate rather than reject or blindly replace, and that
handle common edge cases — newline variants and control characters — correctly.
