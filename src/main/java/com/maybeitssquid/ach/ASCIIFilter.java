 ...
 * <p>
 * This class processes Unicode codepoints, retaining them only if they are within the ASCII range (codepoints less
 * than 128) and do not belong to any of the blocked Unicode categories defined at construction
 * (see {@link Character#getType(int)}). Codepoints that do not meet these criteria are transformed into an empty string.
 */
public class ASCIIFilter implements IntFunction<CharSequence> {
     ...
     * @return a string containing the character if it is a permitted ASCII character; otherwise, an empty string.
     */
    @Override
    public CharSequence apply(final int value) {
        return value < ASCII && !blockTypes.contains(Character.getType(value)) ? Character.toString(value) : "";
    }
}
