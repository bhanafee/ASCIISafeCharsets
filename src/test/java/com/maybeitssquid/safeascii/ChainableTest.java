...
/**
 * Unit tests for the {@link Chainable} class with a focus on its {@link Chainable#apply(int)}
 * method. This tests the core functionality of chaining character transformations using custom
 * processing logic.
 */
class ChainableTest {
  ...
  static class UppercaseChainable extends Chainable {
    ...
    protected CharSequence process(int codepoint) {
      return Character.isLetter(codepoint)
          ? Character.toString(codepoint).toUpperCase()
  ...
  void applyWithEmptyProcessResult() {
    Chainable chainable =
        new Chainable(delegate -> "ignored") {
          @Override
          protected CharSequence process(int codepoint) {
  ...
  void applyWithSingleCharacterResult() {
    ...
    assertEquals(
        "X", result, "Expected delegate output to be returned for single-character process result");
  }

  @Test
  void applyWithMultipleCharacterResult() {
    Chainable chainable =
        new Chainable(codepoint -> String.valueOf((char) (codepoint + 1))) {
          @Override
          protected CharSequence process(int codepoint) {
    ...
    assertEquals(
        "bc",
        result,
        "Expected delegated transformation for each character in the processed result");
  }
  ...
  void applyWithUnicodeCodePoints() {
    Chainable unicodeChainable =
        new Chainable(Character::toString) {
          @Override
          protected CharSequence process(int codepoint) {
    ...
    assertEquals(
        "\uD83D\uDE00", result, "Expected surrogate pair to be handled correctly in buffering");
  }
}
